...
dataImportExp2 <- function(dataFile, normalize=TRUE, delim = ',', graphTypes){
  #Load data
  rawdata <- read.csv(dataFile, sep=delim)
  #remove empty rows
  rawdata <- rawdata[!grepl("NULL",rawdata$task_end) & !grepl("NULL",rawdata$task_end),] 
  ...
  for (i in 1:sampleSize){
    ...
    ##Grid data
    experimentData <- fromJSON(as.character(dat$experimentData))
    #use left-right order to determine if choice was correct (1) or incorrect (0)
    
    #build dataframe using simple parts of the data
    dummy <-data.frame(id = rep(id,36), age = rep(age,36), gender = rep(gender, 36), bonus = rep(bonus, 36), duration = rep(duration, 36), comprehensionTries = rep(experimentData$comprehensionTries,36),
                       graphOrder = experimentData$afcGraphs, choice = experimentData$choice, leftRightChoice = experimentData$leftRightChoice,
                       confidenceJudgments = experimentData$confidenceJudgments, graphType = graphTypes[experimentData$afcGraphs+1])
...
#Issue with fromJSON where unequal vectors coerce the output to be a list instead of a matrix 
#experimentData$tscollect <-do.call(rbind, experimentData$tscollect)
#experimentData$xcollect <- do.call(rbind, experimentData$xcollect)
#experimentData$zcollect<- do.call(rbind, experimentData$zcollect)
#experimentData$zscaledcollect<- do.call(rbind, experimentData$zscaledcollect) 
